...
class TransaksiController extends Controller
{
    ...
    /**
     * TUGAS 2: Method untuk Halaman Laporan Transaksi
     */
    public function laporan(Request $request)
    {
        // Ambil data anggota untuk dropdown filter
        $anggotas = Anggota::orderBy('nama')->get();
        
        // Inisiasi query
        $query = Transaksi::with(['anggota', 'buku'])->latest();

        // 1. Filter Range Tanggal (dari - sampai)
        if ($request->filled('start_date') && $request->filled('end_date')) {
            $query->whereBetween('tanggal_pinjam', [$request->start_date, $request->end_date]);
        }

        // 2. Filter Status (Semua / Dipinjam / Dikembalikan)
        ...
        // Hitung total transaksi & denda sesuai spesifikasi tugas
        $totalTransaksi = $transaksis->count();
        $totalDenda = $transaksis->sum('denda');

        // Fitur Export PDF
        if ($request->has('export') && $request->export == 'pdf') {
            // Pastikan kamu sudah menginstall barryvdh/laravel-dompdf
            $pdf = Pdf::loadView('transaksi.laporan-pdf', compact('transaksis', 'totalTransaksi', 'totalDenda'));
    ...
    {
        // Pastikan format $tanggalDikembalikan adalah Carbon instance
        $tglDikembalikan = Carbon::parse($tanggalDikembalikan);
        $tglKembaliSeharusnya = Carbon::parse($transaksi->tanggal_kembali);

        $hariTerlambat = $tglKembaliSeharusnya->diffInDays($tglDikembalikan, false);
